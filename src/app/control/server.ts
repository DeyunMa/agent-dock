...
import { loadConfig } from "../../router/core/config.js";
import {
...
export async function runControlServer(options: ControlServerOptions): Promise<void> {
  ...
  process.stdout.write(`[agent-dock] control API listening on http://${host}:${port}\n`);

  await new Promise<void>((resolve) => {
    const parentPid = Number.parseInt(process.env.AGENT_DOCK_CONTROL_PARENT_PID ?? "", 10);
