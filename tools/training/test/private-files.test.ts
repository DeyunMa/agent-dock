...
test("private output rejects a pre-existing symlink component", async () => {
  const root = await mkdtemp(resolve(tmpdir(), "agent-dock-private-"));
  try {
...
test("private atomic writes use owner-only file and directory modes", async () => {
  ...
  try {
    ...
    const localPath = resolve(
      process.cwd(),
      "tools/training/work/private-files-test/output.jsonl",
    );
    ...
    await rm(resolve(process.cwd(), "tools/training/work/private-files-test"), {
      recursive: true,
      force: true,
    });
  } finally {
