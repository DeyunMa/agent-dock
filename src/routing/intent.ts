...
const EXPLICIT_ACTION = /(?:(?:帮我|请|直接|现在|那就|就按|按照|开始|继续).{0,30}(?:做|改|修改|实现|修复|新增|添加|删除|清理|安装|运行|执行|启动|部署|发布|提交|推送|创建|生成|制作|写入|迁移|同步|替换|调整|优化|完成))|(?:(?:做|改|修改|实现|修复|新增|添加|删除|清理|安装|运行|执行|启动|部署|发布|提交|推送|创建|生成|制作|写入|迁移|同步|替换|调整|优化|完成).{0,24}(?:一下|吧|并验证|后.{0,12}(?:验证|测试|运行|跑)|完成|就行|即可)[。！!\s]*$)/iu;
