...
import type {
  CodexThreadCatalog,
  CodexThreadSummary,
} from "../src/control/codex-thread-catalog.js";
...
test("status ignores a late stale feed event when a newer turn is audited", async (t) => {
  const directory = await mkdtemp(join(tmpdir(), "codex-router-control-order-"));
  const configPath = join(directory, "router.toml");
  const auditFile = join(directory, "events.jsonl");
  await writeFile(
    configPath,
...
test("status enriches the exact triggering thread by id", async (t) => {
  const directory = await mkdtemp(join(tmpdir(), "codex-router-control-thread-"));
  const configPath = join(directory, "router.toml");
  const auditFile = join(directory, "events.jsonl");
  await writeFile(
    configPath,
  ...
  const threadCatalog = new FakeThreadCatalog({
    "thread-exact": {
      id: "thread-exact",
      name: "Router 时序修复",
      cwd: "/Users/mdy/Code/codex-router",
