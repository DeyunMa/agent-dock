...
project_dir="${0:A:h:h}"
resources_dir="$project_dir/macos/CodexRouterBar/Resources"
source_icon="$resources_dir/AppIconSource.png"
base_icon="$resources_dir/AppIcon.png"
output_icon="$resources_dir/AppIcon.icns"
work_dir="$(mktemp -d "${TMPDIR:-/tmp}/codex-router-icon.XXXXXX")"
trap 'rm -rf "$work_dir"' EXIT

/usr/bin/swift "$project_dir/scripts/render-macos-icon.swift" "$source_icon" "$base_icon"
