...
export async function runInteractiveProxy(
  ...
  server.on("connection", (socket) => {
    if (client && client.readyState === WebSocket.OPEN) {
      socket.close(1013, "Codex Router accepts one client per session");
      return;
    }
    client = socket;
  ...
  const code = await new Promise<number>((resolve) => {
    frontend.once("error", (error) => {
      process.stderr.write(`[codex-router] Codex CLI failed to start: ${error.message}\n`);
      resolve(127);
    });
  ...
  if (code !== 0 && backendError) {
    process.stderr.write(`[codex-router] app-server diagnostic:\n${backendError}\n`);
  }
  return code;
}
