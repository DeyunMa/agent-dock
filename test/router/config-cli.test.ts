...
import { migrateConfigSource } from "../../src/router/core/config-migration.js";
import { parseConfig } from "../../src/router/core/config.js";
import {
  extractExecPrompt,
  hasExplicitRoutingArgument,
  injectExecRoute,
  topLevelCommand,
} from "../../src/router/adapters/codex-arguments.js";

test("example config exposes category, model, effort and fast knobs", async () => {
  const source = await readFile(
    new URL("../../resources/router/router.toml.example", import.meta.url),
    "utf8",
  );
