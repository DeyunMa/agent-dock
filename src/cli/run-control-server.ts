...
export async function runControlServerCommand(args: string[], version: string): Promise<number> {
  ...
  if (!Number.isInteger(port) || port < 1 || port > 65_535) {
    process.stderr.write("codex-router control-server: --port must be between 1 and 65535\n");
    return 2;
  }
  if (host !== "127.0.0.1" && host !== "localhost") {
    process.stderr.write("codex-router control-server only supports loopback hosts\n");
    return 2;
  }
